...
export default function ExamInstructionPage() {
  ...
  useEffect(() => {
    ...
    const fetchExamDetails = async () => {
      try {
        const response = await fetch(`https://codeguard-server-side-1.onrender.com/api/rooms/${roomId}/exam-details`);
    ...
    if (studentId && !storedName) {
      axios.get(`https://codeguard-server-side-1.onrender.com/api/students/${studentId}`)
        .then(res => {
          if (res.data?.name) {

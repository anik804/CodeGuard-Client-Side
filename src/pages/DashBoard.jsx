    ...
    const fetchExaminerAnalytics = async () => {
      try {
        ...
        const [studentsPerExamRes, roomsCreatedRes] = await Promise.all([
          fetch(`https://codeguard-server-side-1.onrender.com/api/analytics/examiner/students-per-exam?examinerUsername=${encodeURIComponent(username)}`),
          fetch(`https://codeguard-server-side-1.onrender.com/api/analytics/examiner/rooms-created?examinerUsername=${encodeURIComponent(username)}`)
        ]);
